  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance
          ...
          .where('orderStatus.pending', isEqualTo: true)
          .snapshots(),
                ...
                children: [
                    ...
                    children: [
                      ...
                      Text(
                        "${orderModel.orderDate
                            ?.toDate()
                            .formateDate} | ${orderModel.orderDate
                            ?.toDate()
                            .hour}-${orderModel.orderDate
                            ?.toDate()
                            .minute}-${orderModel.orderDate
                            ?.toDate()
                            .second}",
                        style: const TextStyle(
                    ...
                    children: [
                      StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                          stream: FirebaseFirestore.instance
                              ...
                              .snapshots(),
                          builder: (context, AsyncSnapshot<DocumentSnapshot<Map<
                              String,
                              dynamic>>> snapshot) {
                        ...
                        child: Row(
                          children: [
                            TextButton(
                              onPressed: () {},
                              child: Text(
                                  '${orderModel.paymentMode?.cashOnDelivery == true
                                      ? 'cod'
                                      : orderModel.paymentMode!.online == true
                                      ? 'online'
                                      : 'dj'}',
                                  style: TextStyle(
                                      color: Colors.white,
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          TextButton(child: const Text('conform',style: TextStyle(
                              fontSize: 15, fontWeight: FontWeight.w700,color: Colors.green)),onPressed: () {
                            AppFireStoreDatabase(collection: 'orders').update(
                              data: {
                                  "orderStatus.confirmed": true,'orderStatus.pending':false}, // Remove the unnecessary string interpolation
                              doc: orderModel.orderId.toString(),
                            );
                          },),
                          TextButton(child: const Text('cancel',style: TextStyle(
                              fontSize: 15, fontWeight: FontWeight.w700,color: Colors.red)),onPressed: () {
                            AppFireStoreDatabase(collection: 'orders').update(
                              data: {
                                "orderStatus.canceled": true,'orderStatus.pending':false}, // Remove the unnecessary string interpolation
                              doc: orderModel.orderId.toString(),
                            );
                          },),
                        ],
